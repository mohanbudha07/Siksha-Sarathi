...
function AdminSchoolSetup() {
  const [data, setData] = useState({ classes: [], subjects: [], teachers: [], students: [], assignments: [] })
  const [classForm, setClassForm] = useState({ name: '', grade: '', section: 'Default' })
  const [subjectForm, setSubjectForm] = useState({ name: '', code: '' })
  const [assignment, setAssignment] = useState({ teacher_user_id: '', class_id: '', subject_id: '', is_class_teacher: false })
  ...
  return <div className="school-setup-page">
    ...
    <div className="school-form-grid">
      ...
      <form onSubmit={(event) => { event.preventDefault(); run(() => api.post('/admin/teacher-assignments', { ...assignment, teacher_user_id: Number(assignment.teacher_user_id), class_id: Number(assignment.class_id), subject_id: Number(assignment.subject_id) }), 'Teacher assignment created.', () => setAssignment({ teacher_user_id: '', class_id: '', subject_id: '', is_class_teacher: false })) }}>
        <h2>Assign teacher</h2><label>Teacher<select value={assignment.teacher_user_id} onChange={(event) => setAssignment({ ...assignment, teacher_user_id: event.target.value })} required><option value="">Select teacher</option>{data.teachers.map((item) => <option key={item.id} value={item.id}>{item.username}</option>)}</select></label><label>Class<select value={assignment.class_id} onChange={(event) => setAssignment({ ...assignment, class_id: event.target.value })} required><option value="">Select class</option>{data.classes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>Subject<select value={assignment.subject_id} onChange={(event) => setAssignment({ ...assignment, subject_id: event.target.value })} required><option value="">Select subject</option>{data.subjects.map((item) => <option key={item.id} value={item.id}>{item.name}{item.code ? ` (${item.code})` : ''}</option>)}</select></label><label className="school-check"><input type="checkbox" checked={assignment.is_class_teacher} onChange={(event) => setAssignment({ ...assignment, is_class_teacher: event.target.checked })} />Make this teacher responsible for class attendance</label><button disabled={busy}>Save Assignment</button>
      </form>
    </div>
